...
class EventAdapter(private val callback: (Event) -> Unit)
	...
	inner class ViewHolder(private val binding: MasterEventBinding) : RecyclerView.ViewHolder(binding.root) {
		init {
	...
	companion object {
		val EVENT_COMPARATOR = object : DiffUtil.ItemCallback<Event>() {
			...
			override fun areItemsTheSame(oldItem: Event, newItem: Event) = oldItem.eventId == newItem.eventId
		}
	}
}